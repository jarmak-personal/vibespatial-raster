"""NVRTC shared-memory tiled convolution kernels for focal raster operations.
...
The convolution supports arbitrary KxK kernels (max 25x25 to fit shared memory).
Tile dimensions are compile-time defines (TILE_W, TILE_H) set to 16x16.
Shared memory layout:
  - data tile: (TILE_H + 2*PAD_Y) * (TILE_W + 2*PAD_X + 1) doubles  (+1 bank padding)
  - nodata tile: (TILE_H + 2*PAD_Y) * (TILE_W + 2*PAD_X) uint8s (1/8 cost of data)
  - kweights: kh * kw doubles  (kernel weights loaded once per block)
"""
